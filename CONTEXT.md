...
- **Q26.1 — The classification is ONE owner, every recorder (2026-09-05).** The item-vs-lane rule is not per-route: `cs_uk_api/health.py` owns the single classification table (`ITEM_VERDICT_CODES` / `LANE_VERDICT_CODES`) and `record_verdict(provider_id, code)` is the one recording entry point every failure path uses — the facade stream path, the native `upstream_guard`, the home-snapshot browse recorders, and the search fan-out. Item verdicts (`gated`, `not_found`, `invalid_translation`, `translation_missing`) never flip lane health on ANY surface (the #373 finding generalized); `parse_failed` and all transport codes are lane faults. The drift test (`tests/test_verdict_drift.py`) fails CI when a new `ProviderError`/`ProviderFailure` code appears without a verdict class.
