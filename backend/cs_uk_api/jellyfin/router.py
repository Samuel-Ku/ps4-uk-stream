...
from .identity import _server_id
...
from .resolution import (
    ...
    _genres_for_group,
    _parse_genre_ids,
    _parse_include_types,
    _poster_for,
    _resolve_view_row_type,
    _split_season_suffix,
    _user_data,
    _view_id_for,
    _view_id_for_item,
    _year_for_group,
...
def _person_filmography(
    ...
    The state walk — person-name recovery, the profile-store match, the
    home-snapshot group scan, the IncludeItemTypes form filter — lives on
    the resolution half (``resolution.person_filmography_pairs``); this
    route-side wrapper assembles the DTOs and paginates.
    """
    pairs = person_filmography_pairs(person_ids, include_item_types)
    server_id = _server_id()
    dtos = [_item_dto(row, it, server_id) for row, it in pairs]
    total = len(dtos)
    end = None if limit is None else start_index + limit
    return BaseItemDtoQueryResult(
        Items=dtos[start_index:end],
...
# The Dashboard surface (spec #280) registers HERE — not at the tail with
# the other moved conversations — because its ``/Items/Counts`` route must
# precede the ``/Items/{item_id}`` declaration below. FastAPI matches in
# registration order, so a tail registration would let ``Counts`` be
# swallowed as an item id and 404. This is the one table position in the
# whole split that is not inert; see jellyfin/dashboard.py.
register_dashboard(router)
...
# middleware.
#
# The Dashboard surface is the exception, and it deliberately registers
# MID-FILE: `register_dashboard` is called above, immediately before the
# /Items/{item_id} declaration, because /Items/Counts must precede it in
# the table. See jellyfin/dashboard.py.
