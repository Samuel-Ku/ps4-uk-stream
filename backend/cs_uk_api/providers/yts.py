...
# (The magnet→session policy — candidate parsing, quality ordering,
# bounded dead-swarm fallback, magnet convention, wire shape — lives in
# :mod:`cs_uk_api.torrent_lane`, the lane conversation this provider
# drives; this module keeps the popcorn catalog dialect.)
...
class YtsProvider(BaseProvider):
    ...
    async def _stream_movie(
        ...
        try:
            result = await ensure_any_session(engine, entries, file_hint=None)
        ...
        return torrent_stream_response(result)
    ...
    async def _stream_episode(
        ...
        try:
            result = await ensure_any_session(
    ...
    def _record_torrents(self, external_id: str, movie: PopcornMovie) -> None:
        candidates = torrent_candidates(movie.torrents)
        if not candidates:
            return
