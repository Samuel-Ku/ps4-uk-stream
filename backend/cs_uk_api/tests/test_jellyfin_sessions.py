...
def _isolate_playback() -> None:
    ...
    routes store nothing (D8) — this reset is belt-and-braces isolation
    through the same catalog-state store helpers the suite hygiene uses
    (conftest, #330), never a direct mutation."""
    install_profiles({})
    clear_playback()
    yield
    install_profiles({})
