...
async def test_klontv_search_parses_results():
    ...
    with respx.mock(assert_all_called=True) as router:
        router.post("https://klon.fun/").respond(200, text=search_html)
    ...
    assert all(r.url.startswith("https://klon.fun/") for r in results)
...
async def test_klontv_browse_films_page1():
    ...
    with respx.mock(assert_all_called=True) as router:
        router.get("https://klon.fun/filmy/").respond(200, text=listing_html)
        async with httpx.AsyncClient() as http:
    ...
    assert all(
        r.url.startswith("https://klon.fun/filmy/") or r.url.startswith("https://klon.fun/serialy/")
        for r in results
    )
...
async def test_klontv_browse_series_page1():
    ...
    with respx.mock(assert_all_called=True) as router:
        router.get("https://klon.fun/serialy/").respond(200, text=listing_html)
        async with httpx.AsyncClient() as http:
    ...
    assert all(r.url.startswith("https://klon.fun/serialy/") for r in results)
    assert has_next is True
...
async def test_klontv_browse_films_last_page():
    ...
    listing_html = _fixture("films_listing.html")
    with respx.mock(assert_all_called=True) as router:
        router.get("https://klon.fun/filmy/page/400/").respond(200, text=listing_html)
        async with httpx.AsyncClient() as http:
...
async def test_klontv_content_movie_parses_title_poster_player():
    ...
    with respx.mock(assert_all_called=True) as router:
        router.get(
            "https://klon.fun/filmy/11719-duna-chastyna-druga.html"
    ...
    assert c.poster.startswith("https://klon.fun/uploads/")
...
async def test_klontv_content_series_parses_seasons():
    ...
    with respx.mock(assert_all_called=True) as router:
        router.get("https://klon.fun/serialy/8431-duna.html").respond(
