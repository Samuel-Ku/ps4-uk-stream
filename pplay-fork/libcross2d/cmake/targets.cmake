...
if (PLATFORM_PS4 AND NOT COMMAND add_self)
    ...
    function(add_pkg PROJECT ROMFS_DIR TITLE_ID TITLE VERSION)
        ...
        # PkgTool.Core expects a pkg.gp4 project file alongside the
        # staged tree that `pplay.pkgtree` (issue #95) materialised.
        # The gp4 is generated by gpkg_gen.cmake (issue #96), which
        # enumerates every file under ${PKG_DIR} — no more 4-file
        # hardcoded list.
        set(GP4_PATH "${CMAKE_CURRENT_BINARY_DIR}/pkg.gp4")
        set(GPKG_SCRIPT "${CMAKE_CURRENT_LIST_DIR}/gpkg_gen.cmake")
        add_custom_target(${PROJECT}_pkg
                DEPENDS ${PROJECT}.pkgtree
                # Regenerate pkg.gp4 from the current staged tree on
                # every build (so adding a file to data/ in a future
                # PR doesn't need a regenerate step).
                COMMAND ${CMAKE_COMMAND}
                    -DPKG_DIR=${PKG_DIR}
                    -DGP4_PATH=${GP4_PATH}
                    -DTITLE_ID=${TITLE_ID}
